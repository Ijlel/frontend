...
const UsersPage = () => {
  // Destructure custom hook to manage users data and operations (CRUD)
  const {
    users,
    userSelected,
    setUserSelected,
  ...
  //  Function to handle form submission for creating or updating an user
  const handleFormSubmit = (event) => {
    ...
    if (modalAction === "create") {
      createUser(userData)
        .then(() => {
          handleClose();
          getAllUsers(); // Refresh list after creation
        })
        .catch((err) => {
      ...
      updateUser(userSelected._id, userData)
        .then(() => {
          handleClose();
          getAllUsers(); // Refresh list after update
        })
        .catch((err) => {
  ...
  //  Function to handle deletion of an user with confirmation
                ...
                currentItems.map((user) => (
                  <tr key={user._id}>
                    ...
                    <OverlayTrigger
                      placement="top"
                      overlay={
                        <Tooltip id="lastNameUser-tooltip">{user.lastName}</Tooltip>
                      } // Show full last name on hover
                    >
                      <td>{truncateText(user.lastName, 5) || "-"}</td>
                    </OverlayTrigger>

                    <OverlayTrigger
                      placement="top"
                      overlay={
                        <Tooltip id="genderUser-tooltip">{user.gender}</Tooltip>
                      } // Show full gender on hover
                    >
                      <td>{truncateText(user.gender, 5)}</td>
                    </OverlayTrigger>
                    ...
                    >
                      <td>{truncateText(user.email, 20)}</td>
                    </OverlayTrigger>
                    <td>
                      <OverlayTrigger
                        placement="top"
